    ...
    <div className="min-h-screen bg-[#F9F8F3] text-[#1C1B18]">
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-3">
              ...
              <span className="font-serif text-xl font-bold tracking-tight">
                ATELIER <span className="text-[#A37D4C]">Product CRUD</span>
              </span>
            </div>

            <div className="flex items-center space-x-6">
              <nav className="flex items-center space-x-4 text-xs font-semibold">
                <Link href="/admin" className="text-[#A39E93] hover:text-white transition-colors py-2">
                  Dashboard &amp; Transaksi DOKU
                </Link>
                <Link href="/admin/products" className="text-[#A37D4C] border-b-2 border-[#A37D4C] py-2">
                  Kelola Produk (CRUD)
                </Link>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button
            ...
            className="inline-flex items-center space-x-2 bg-[#0F4C3A] hover:bg-[#1C1B18] text-white text-xs font-bold px-5 py-3 rounded-xl transition-all shadow-sm cursor-pointer"
          >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-[#E6E3D8] text-[#1C1B18]">
                {products.length > 0 ? (
                  products.map((item) => (
                    <tr key={item.id} className="hover:bg-[#F9F8F3]/60 transition-colors">
                      
                      <td className="p-4">
                        <div className="relative w-12 h-10 rounded-lg overflow-hidden bg-[#F3F0E6] border border-[#E6E3D8]">
                          <Image src={item.previewImage} alt={item.title} fill className="object-cover" />
                        </div>
                      </td>

                      <td className="p-4">
                        <span className="font-serif font-bold text-sm text-[#1C1B18] block">{item.title}</span>
                        <span className="text-[10px] text-[#5C5953] font-mono">/{item.slug}</span>
                      </td>

                      <td className="p-4">
                        <span className="bg-[#1C1B18] text-[#F9F8F3] text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full">
                          {item.category}
                        </span>
                      </td>

                      <td className="p-4 font-serif font-bold text-[#0F4C3A] text-sm">
                      ...
                      <td className="p-4 text-right space-x-2">
                        <button
                          ...
                          className="p-2 text-[#5C5953] hover:text-[#0F4C3A] hover:bg-[#F9F8F3] rounded-lg transition-colors cursor-pointer"
                          title="Edit Produk"
                        >
                        ...
                        <button
                          ...
                          className="p-2 text-[#5C5953] hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors cursor-pointer"
                          title="Hapus Produk"
                        >
                  ...
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-[#5C5953]">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs overflow-y-auto">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border border-[#E6E3D8] overflow-hidden my-8">
            
            <div className="bg-[#1C1B18] text-[#F9F8F3] px-6 py-4 flex items-center justify-between">
            ...
            <form onSubmit={handleSaveProduct} className="p-6 space-y-4 text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block font-bold text-[#1C1B18] uppercase tracking-wider mb-1">
                    Judul Produk *
                  </label>
                  <input
                    type="text"
                    ...
                    className="w-full px-3 py-2.5 rounded-lg border border-[#E6E3D8] text-xs text-[#1C1B18]"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2.5 rounded-lg border border-[#E6E3D8] text-xs text-[#1C1B18] font-mono"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2.5 rounded-lg border border-[#E6E3D8] text-xs text-[#1C1B18] bg-white font-semibold"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2.5 rounded-lg border border-[#E6E3D8] text-xs font-mono"
                  />
                </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 rounded-lg border border-[#E6E3D8] text-xs"
                />
              </div>

              <div className="pt-4 border-t border-[#E6E3D8] flex justify-end space-x-3">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="px-5 py-2.5 rounded-lg border border-[#E6E3D8] text-[#5C5953] font-semibold hover:bg-gray-50"
                >
                  Batal
                </button>
                <button
                  type="submit"
                  className="px-6 py-2.5 rounded-lg bg-[#0F4C3A] hover:bg-[#1C1B18] text-white font-bold transition-all shadow-sm"
                >
