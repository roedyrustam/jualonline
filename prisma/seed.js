...
async function main() {
  console.log('Seeding database with sample digital products, coupons, admin account, and verified reviews...');
  ...
  for (const item of products) {
    ...
    // Seed Sample Paid Order & Review for each product
    const orderNumber = `DKU-SEED-${createdProduct.slug.slice(0, 10).toUpperCase()}`;
    const order = await prisma.order.upsert({
      ...
      create: {
        ...
        status: 'PAID',
        dokuInvoiceNumber: `INV-SEED-${createdProduct.slug.slice(0, 8).toUpperCase()}`,
    ...
    if (!existingReview) {
      await prisma.review.create({
        data: {
          ...
          customerName: 'Budi Pratama',
          rating: 5,
          comment: 'Kualitas produk sangat memuaskan, kodenya bersih dan dokumentasinya mudah dipahami. Pengiriman file instan via DOKU.',
          isVerified: true,
        },
      });
    }
